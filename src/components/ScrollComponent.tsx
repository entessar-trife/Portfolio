...
const ScrollComponent = () => {
  ...
  return (
    <div>
      <button
        onClick={scrollToTop}
        className={`fixed right-5 bottom-5 z-50 shadow-md
        bg-white
      ...
      >
        <IoIosArrowUp className="text-purple-500 text-xl" />
      </button>

      <button
        onClick={scrollToBottom}
        className={`fixed bottom-5 z-50 shadow-md
        bg-white
          w-10 h-10 flex items-center justify-center
        rounded-full cursor-pointer 
        duration-300 hover:-translate-y-0.5 
      ...
      >
        <IoIosArrowDown className="text-purple-500 text-xl" />
      </button>
    </div>
  );
};
